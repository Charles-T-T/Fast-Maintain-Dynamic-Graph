			 ...
			 std::vector<affected_label> &al1, std::vector<pair_label> *al2, ThreadPool &pool_dynamic, std::vector<std::future<int>> &results_dynamic)
{
	...
	for (const auto &al : al1){
		results_dynamic.emplace_back(pool_dynamic.enqueue([al, L, al2, &instance_graph]
		{
			// TODO debug line
			...
			/*
			std::priority_queue<node_for_DIFFUSE> Q;
			Q.push(node_for_DIFFUSE{ al.first , al.dis });
			while (!Q.empty()) {
				node_for_DIFFUSE top = Q.top();
				Q.pop();
				// (*L)[top.index][al.second].distance == INF;
				insert_sorted_two_hop_label((*L)[top.index], al.second, MAX_VALUE);
				al2->push_back(pair_label{ top.index, al.second });
				for (auto& x_n : instance_graph[top.index]) {
					if (al.second < x_n.first) { // rank(v) > rank(x_n)
						if (abs(search_sorted_two_hop_label((*L)[x_n.first], al.second) - top.disx - x_n.second) < 1e-5) {
							Q.push(node_for_DIFFUSE{ x_n.first, top.disx + x_n.second });
						}
					}
				}
			}
			 ...
			 std::vector<pair_label> &al2, std::vector<affected_label> *al3, ThreadPool &pool_dynamic, std::vector<std::future<int>> &results_dynamic)
{
	...
	std::mutex mutex_al3;
	std::mutex mutex_PPR;
	for (auto each_pair : al2)
	{
		results_dynamic.push_back(pool_dynamic.enqueue([each_pair, &al2, al3, L, PPR, &instance_graph]{

		// TODO debug line
		std::cout << "spread2" << std::endl;

		/*
		//std::vector<int> nodes = PPR[each_pair.first][each_pair.second].second;
		std::vector<int> nodes = PPR_retrieve(*PPR, each_pair.first, each_pair.second);
		nodes.push_back(each_pair.second);
		for (int i = 0; i < nodes.size(); i++)
		{
			int each_t = nodes[i];
			// int r_t = (*L)[each_t].back().vertex;
			// int r_x = (*L)[each_pair.second].back().vertex;
			int x = each_pair.first;
			if (each_t < x)
			{
				double dis_xt = 1e6;
				for (auto neighbor_x : instance_graph[x])
				{
					double newDist = neighbor_x.second + search_sorted_two_hop_label((*L)[neighbor_x.first], each_t);
					dis_xt = std::min(dis_xt, newDist);
				}
				auto query_result = graph_hash_of_mixed_weighted_two_hop_v1_extract_distance_no_reduc2(*L, x, each_t);
				double query_distance = query_result.first;
				if (query_distance > dis_xt)
				{
					affected_label aff_node = affected_label(x, each_t, dis_xt);
					mutex_al3.lock();
					al3->push_back(aff_node);
					mutex_al3.unlock();
				}
				else
				{
					int hc = query_result.second;
					{
						mtx_5952[each_pair.first].lock();
						PPR_insert(*PPR, each_pair.first, hc, each_t);
						mtx_5952[each_pair.first].unlock();
					}
					{
						mtx_5952[each_t].lock();
						PPR_insert(*PPR, each_t, hc, each_pair.first);
						mtx_5952[each_t].unlock();
					}
				}
			}
			if (x < each_t)
			{
				double dis_tx = 1e6;
				for (auto neighbor_t : instance_graph[each_t])
				{
					double newDist = neighbor_t.second + search_sorted_two_hop_label((*L)[neighbor_t.first], each_pair.first);
					dis_tx = std::min(dis_tx, newDist);
				}
				auto query_result = graph_hash_of_mixed_weighted_two_hop_v1_extract_distance_no_reduc2(*L, each_t, each_pair.first);
				double query_distance = query_result.first;
				if (query_distance > dis_tx)
				{
					mutex_al3.lock();
					affected_label aff_node = affected_label(each_t, each_pair.first, dis_tx);
					al3->push_back(aff_node);
					mutex_al3.unlock();
				}
				else
				{
					...
					//(*PPR)[each_pair.first][hc].second.push_back(each_t);
					//(*PPR)[each_t][hc].second.push_back(each_pair.first);
				}
			}
